import { Div, Img, LI, RxLI, TextArea, UL, RxFragment, PointerButton } from 'reactronic-front'
...
export function TaskLine(id: string, task: Task, app: App) {
  ...
  return (RxLI('Task' + id, null, e => {
    ...
    if (task.notCompleted) {
      RxFragment('TaskLineReaction' + id, null, () => {
        Reactronic.configureCurrentMethod({ trace: TraceLevel.Suppress })
        const pointer = app.sensors.pointer
        if (pointer.down === PointerButton.Left && pointer.sensorDataList[1] === e.id && pointer.draggableObject === undefined) {
          pointer.draggableObject = e.id
          //e.classList.add('selected')
          app.currentItemID = app.taskList.indexOf(task)
          app.nextItemId = app.currentItemID
          console.log('start')
          console.log(pointer.draggableObject)
        }

        const dragging = pointer.draggingObject === e.id
        if (dragging) {
          console.log('over')
          console.log(app.currentItemID + ' ' + app.nextItemId)
        }

        if (pointer.droppedObject === e.id) {
          console.log('end')
          //e.classList.remove('selected')
          app.swapTasks()
        }
